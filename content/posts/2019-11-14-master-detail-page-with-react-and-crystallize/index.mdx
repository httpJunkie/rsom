...
Create a directory named `components` and inside the `src` directory, we will add all of the following files there:
...
We will start by updating the code in the `src/App.js` file, as we add code to each files, I will explain what each file is doing. 
...
In `App.js`, we import React Router's `BrowserRouter` and `Route` components for providing route data and declarative route definitions. We also use the `Switch` component to in case a user navigates to a page other than the root `'/'` or `'Illustrations'` routes. In this case they will see our `Error404` component as it is the last route defined in the `Switch`. 
...
This component that displays if no other routes are matched. We display the location they ended up at, and a link to get back home.
...
We will pass in our `IllustrationMasterdetail` component and a GraphQL URI to a higher-order-component and it will wrap our component in an **Apollo Provider** giving us the ability to make queries from our `IllustrationMasterDetail` page or any of it's child components. 
...
All we do here is grab our `illustrations` from the data passed into as a prop of the same name and use that data to render out a list of `Link` components from **React Router** that when clicked, will change the URL path.

With that file in place we need to fetch the data.
...
We now will import the above code into `IllustrationMasterDetail` along with some other imports and code to help us retrieve and process the data:
...
We have only updated the `Illustrations` Route and gave it a new route with slug to match on the the product link. Let's run the project!
