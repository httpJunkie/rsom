...
One componeI am using a higher order component that I created so that I can wrap any component with my GraphQL provider, I'd like to get feedback on this if you are interested, but it's something I thought that would make working with the Apollo Client in our project easier, especially if we needed more than one component to use teh GraphQL cleint.

[httpJunkie/withApolloProvider.js](https://gist.github.com/httpJunkie/d365b40f0da862569d720c97f54b1fe3)
```JSX
...
In our project with have a master detail component relationship, when we are at the route for `/airlines`, we display a component called `airlines.jsx`, 
it uses React Router and when no specific `arlineId` is present in the URL:

`http://localhost:3000/airlines`

Our Airline component will display a list of airline on the left side of the page and a message on the right side of the page that says to select an airline. 
Selecting an airline from the menu will load details on the right side of the page.
