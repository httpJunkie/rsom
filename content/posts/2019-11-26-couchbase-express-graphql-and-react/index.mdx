...
I am using a higher order component that I created so that I can wrap any component with my GraphQL provider, in this tutorial we are only doing that with one component (Airlin.jsx)/
But if we had more components that needed to use the client we could easily do so with this higher order component.
