...
GraphQL provides granular control over data allowing developers to be flexibile without unnecessary calls to the server. Developers love it because it provides an alternative to working with REST APIs, as well it can aid in simply working with REST API's instead of building complex gateway APIs that are tough to version without constant breaking changes. GraphQL provides a higher level of flexibility in API development and I hate to say that you just need to get started and build something with it if you are having any probelms understanding exactly what it does and how it may help you and your team.

I also wanted to introduce my readers to a NoSQL solution for data storage, Although this project is shallow in how it teaches is each part of this full stack JavaSCript application, it's the first steps you need to take to get started with these technologies and a lot of the stuff you learn can be transferable to other NoSQL technologies.

I'm using [Couchbase Server](https://docs.couchbase.com/server/6.0/sdk/overview.html) as my document datastore. 
Although we only set up one node, Couchbase Server makes it possible to do everything you need from having a basic memory-first NoSQL datastore to cross data center replication, geo-distributed deployments, workload isolation, and much more with unmatched agility and flexibility. 
I know, a bunch of complex concepts and we barely scratch the surface in this tutorial, but you gotta walk before you run and I intend to simply get you up and running with a full stack application in the easiest way possible. 
...
If you are new to all of this, don't we start from a beginner level perspective, if you are familiar, just breeze through the setup and the tutorial will go a lot faster!
