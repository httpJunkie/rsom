...
Step 1. Get the `couchbase` image
...
Now we can run our docker comands:

```bash
docker build -t couchbase-server .
```

Now we will launch our image we just built:

```bash
docker run -d -p 8091-8094:8091-8094 -p 11210:11210 -e CB_ADMIN_USER=Administrator -e CB_ADMIN_PASS=123456 -e CB_BUCKET=default -e CB_BUCKET_PASS= --network="bridge" --name cbs1 couchbase-server
```
